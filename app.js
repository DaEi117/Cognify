...
console.log("Cognify – Full app.js (inkl. XLSX Import/Export, Bearbeiten, Bilder pro Liste via IndexedDB) + Fix: Umlaut-/Unicode-Bildnamen zuverlässig");
...
// ä->ae, ö->oe, ü->ue – robust auch für iOS (NFD-Dekomposition)
function normalizeKeyUmlautToE(input) {
    let s = normalizeUnicodeNFC(input).toLowerCase();
    s = s.replace(/\s+/g, " ").trim();

    // NFD: dekomponierte Umlaute behandeln (a + ¨)
    s = s.normalize("NFD");
...
// ä->a, ö->o, ü->u (nur Diacritics entfernen)
...
// Eindeutig (kollisionsarm): Original (NFC) -> lower/trim -> spaces->_ -> encodeURIComponent
// "schön" -> "sch%C3%B6n"
...
async function countImagesForList(listName) {
    ...
    return new Promise((resolve, reject) => {
        ...
        cursorReq.onsuccess = () => {
            ...
            if (!cursor) {
                // Anzahl echter Dateien (nicht Alias-Keys)
                resolve(unique.size);
                return;
            }
            const item = cursor.value;
            // originalName = echter Dateiname beim Import
            unique.add(item.originalName || item.key);
            cursor.continue();
        };
...
// FIX: Umlaut-Namen + iOS Unicode + KEIN Komplett-Skip bei Key-Kollision
// ==========================
fileImportImages.onchange = async () => {
    ...
    try {
        for (const file of allowed) {
            ...
            // Wichtig: jeden Key separat versuchen (Option A: nicht überschreiben)
            let wroteAny = false;
            for (const k of keys) {
...
// ==========================
// Bilder anzeigen in Abfrage (Lookup über mehrere Keys)
// ==========================
...
function neueFrage() {
    ...
    // Frage-Bild laden und danach zu "Prüfen" scrollen (wenn Layout ändert)
