...
console.log("Cognify – Fix: Scroll nach Start/Prüfen + Buttons immer sichtbar + Bilder pro Liste (IndexedDB) + Umlaut-robuste Bildkeys");
...
// ==========================
// Bild-Key Normalisierung (Umlaut-robust)
// ==========================
function normalizeKeyUmlautToE(input) {
    ...
    // Unicode vereinheitlichen (wichtig für iOS / Files)
    s = s.normalize("NFC");

    // Diacritics weg + nur a-z0-9
...
function candidateKeysForText(text) {
    ...
    const a = normalizeKeyUmlautToE(raw);
    const b = normalizeKeyStripDiacritics(raw);
...
// ==========================
// Bilder importieren (pro Liste) – speichert mehrere Keys (Umlaut-robust)
// ==========================
fileImportImages.onchange = async () => {
    ...
    try {
        for (const file of allowed) {
            ...
            // Option A: nicht überschreiben -> wenn irgendeiner existiert, skip
            let alreadyThere = false;
            for (const k of keys) {
                if (await imageExists(listName, k)) { alreadyThere = true; break; }
            }
        ...
        alert(`Bilder-Import fertig ✅\nImportiert: ${imported}\nÜbersprungen: ${skipped}\n\nHinweis: Für bereits alte Imports ggf. Bilder einmal erneut importieren.`);
        await anzeigenListen();
    } catch (e) {
        console.error(e);
...
// ==========================
// Bilder anzeigen in Abfrage (Umlaut-robuster Lookup)
// ==========================
