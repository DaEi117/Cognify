...
console.log("Cognify – Phase 18 (Blue Minimal UI)");
...
function setAbfrageUIState(state) {
    if (state === "preCheck") {
        ...
        // Abfrage-Box automatisch ins Blickfeld holen
        abfrageBox.scrollIntoView({
            behavior: "smooth",
            block: "center"
        });
...
// ==========================
// Listen anzeigen (Bearbeiten + Export pro Liste)
// ==========================
function anzeigenListen() {
    ...
    listen.forEach(l => {
        ...
        row.append(titel, open, edit, exportJson, exportCsv, fachReset, statReset, del);

        const s = berechneListenStatistik(l);
        const stat = makeMeta(`Einträge: ${s.eintraege} | Antworten: ${s.gesamt} | ✔ ${s.richtig} | ✘ ${s.falsch} | ${s.quote}%`);
    ...
    reader.onload = (evt) => {
        try {
            ...
            for (let i = 1; i < rows.length; i++) {
                const a = normText((rows[i] || [])[0]);
                const b = normText((rows[i] || [])[1]);
                if (!a || !b) continue;
                eintraege.push({ a, b, richtig: 0, falsch: 0, fach: 1 });
            }
