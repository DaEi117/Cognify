...
console.log("Cognify – Bilder pro Liste (IndexedDB) + Anzeige in Abfrage");
...
// - robust, aber exaktes Benennen funktioniert weiterhin (weil wir beide Seiten gleich normalisieren)
// ==========================
function normalizeKey(input) {
    let s = (input ?? "").toString();

    // Zeilenumbrüche/Whitespace vereinheitlichen
    ...
    // Diakritika entfernen (z.B. é -> e)
    s = s.normalize("NFD").replace(/[\u0300-\u036f]/g, "");

    // Alles, was keine Buchstaben/Zahlen ist, wird zu Leerzeichen
    s = s.replace(/[^a-z0-9]+/g, " ").trim();

    // Optional: Leerzeichen -> underscore
    s = s.replace(/\s+/g, "_");
...
// ==========================
// IndexedDB: Bilder speichern (lokal, offline)
// ==========================
const DB_NAME = "cognify_db";
const DB_VERSION = 1;
const STORE_IMAGES = "images";
// keyPath: [listName, key]  (Option A: erstes Bild pro Key)
let dbPromise = null;
...
async function putImage(listName, key, blob, mime, originalName) {
    ...
    return new Promise((resolve, reject) => {
        ...
        // Option A: wenn schon vorhanden, nicht überschreiben
...
// ==========================
// Bild-Komprimierung (Empfehlung #7)
// - max 1024px Kantenlänge
// - JPEG Qualität 0.82
// - PNG bleibt PNG (Resizing ja, aber PNG bleibt groß, daher nur resizing)
// ==========================
...
// Für Bildimport: welche Liste ist gerade Ziel?
...
async function anzeigenListen() {
    ...
    for (const l of listen) {
        ...
        // ✅ NEU: Bilder hinzufügen pro Liste
        ...
        imgAdd.onclick = () => {
            ...
            alert(
                ...
                `Hinweis iPhone: Öffne im Datei-Picker den Ordner der Liste und wähle die Bilder (Mehrfachauswahl).`
            );
            // Desktop kann ggf. Ordnerauswahl, iOS ignoriert es:
            fileImportImages.setAttribute("webkitdirectory", "");
            fileImportImages.setAttribute("directory", "");
            fileImportImages.click();
        };

        // ✅ NEU: Bilder löschen pro Liste
    ...
    reader.onload = (evt) => {
        try {
            ...
            for (let i = 1; i < rows.length; i++) {
                ...
                // Wichtig: Mehrzeilige Zellen sollen bleiben → kein normText hier!
                eintraege.push({ a: aStr, b: bStr, richtig: 0, falsch: 0, fach: 1 });
            }

            if (eintraege.length === 0) return alert("Keine gültigen Einträge gefunden. Bitte sicherstellen: Ab Zeile 2 stehen in Spalte A und B Werte.");
...
fileImportImages.onchange = async () => {
    ...
    try {
        for (const file of allowed) {
            ...
            // Option A: nur erstes Bild pro Key
            if (await imageExists(listName, key)) { skipped++; continue; }
        ...
        alert(`Bilder-Import fertig ✅\nImportiert: ${imported}\nÜbersprungen: ${skipped}\n\nHinweis: Dateiname muss dem Eintrag (Spalte A oder B) entsprechen. (Robustes Matching)`);
        await anzeigenListen();
    } catch (e) {
        console.error(e);
...
function setAbfrageUIState(state) {
    if (state === "preCheck") {
        ...
        // Antwortbild ausblenden bis Prüfen
        hideAnswerImage();
        ...
        abfrageBox.scrollIntoView({ behavior: "smooth", block: "center" });
...
// - Frage-Bild immer (falls vorhanden)
// - Antwort-Bild erst nach Prüfen (falls vorhanden)
// ==========================
...
async function showQuestionImageFor(text) {
    ...
    if (token !== imageRequestToken) return; // race guard
    ...
    hideQuestionImage(); // revoke previous
...
function neueFrage() {
    ...
    // Frage-Bild: immer (falls vorhanden)
    showQuestionImageFor(questionText);
...
// ==========================
// XLSX Export/Import & Listen UI hooks
// ==========================
async function init() {
    // DB warmup
